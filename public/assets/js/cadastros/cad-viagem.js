        ...
        $.each(dados, function (i, obj) {
            ...
            // if($("#freteAd tr").length > 0) {
                ...
                $("#freteAd").append('<tr id="freteTable' + obj.id + '"><td>' + obj.nome + '</td><td>' + obj.tipo + '</td><td>' + obj.identificacao + '</td><td>' + obj.cidade_origem + '</td><td>' + obj.cidade_destino + '</td>' +
                    '<td><input type="text" class="form-control moeda" name="custos['+ obj.id +']" data-prefix="R$"/></td><td><a onclick="removerFrete(' + obj.id + ')" class="remover btn btn-danger btn-sm"><i class="fa fa-trash"></i></a></td></tr>')
